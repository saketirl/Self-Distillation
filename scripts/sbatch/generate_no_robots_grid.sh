#!/bin/bash
# Generate sbatch files for grid search on no_robots dataset
# Uses --proj_no_resync (no resyncing of UBV factorization)
# Dataset: HuggingFaceH4/no_robots (~10k samples, short sequences)

OUTPUT_DIR="scripts/sbatch/no_robots_grid"
mkdir -p "$OUTPUT_DIR"
...
for gc in "${GRAD_CLIPS[@]}"; do
    for et in "${ENERGY_THRESHOLDS[@]}"; do
        for lr in "${LRS[@]}"; do
            ...
            EXP_ID="norobots_gc${gc_safe}_et${et_safe}_lr${lr_safe}"
