#!/bin/bash
# Analyze eigenspectrum for base Qwen model using SDFT (KL divergence) loss
# Uses the same model as both student and reference
# Saves results in eigenspectrum/outputs/qwen2.5-3b-instruct_sdft/layer_*/

set -e

MODEL_PATH="${MODEL_PATH:-/data/saket/continual/Self-Distillation/models/qwen2.5-3b-instruct}"
REF_MODEL_PATH="${REF_MODEL_PATH:-/data/saket/continual/Self-Distillation/models/qwen2.5-3b-instruct}"
OUTPUT_BASE="${OUTPUT_BASE:-/data/saket/continual/Self-Distillation/eigenspectrum/outputs/qwen2.5-3b-instruct_sdft}"
...
echo "=============================================="
echo "Base Model Eigenspectrum Analysis (SDFT Loss)"
echo "=============================================="
echo "Model: ${MODEL_PATH}"
echo "Reference: ${REF_MODEL_PATH}"
echo "Output: ${OUTPUT_BASE}"
echo "Layers: 0 to $((NUM_LAYERS-1))"
echo "Data samples: ${NUM_DATA_SAMPLES}"
echo "Lanczos order: ${LANCZOS_ORDER}"
echo "Loss type: SDFT (KL divergence)"
...
for layer in $(seq 0 $((NUM_LAYERS-1))); do
    ...
    echo ""
    echo "=============================================="
    echo "[Layer ${layer}/${NUM_LAYERS}] Analyzing with SDFT loss..."
...
if len(layers) > 0:
    ...
    axes[0].set_title('Top Eigenvalue Across Layers (Base Model, SDFT Loss)')
    ...
    axes[1].set_title('Effective Rank Across Layers (Base Model, SDFT Loss)')
