#!/usr/bin/env python3
"""
Generate heatmaps for alignment mass metrics (V vectors) from CSV data.

For each matrix and comparison:
- X-axis: layer
- Y-axis: k value
- Color: diag_mass, off_diag_mass, or diag_ratio for V vectors
"""
...
def plot_single_matrix_all_metrics(
    ...
    """
    Plot heatmaps for a single matrix showing all metrics for V vectors.
    ...
    metrics_info = [
        ('V_diag_mass', 'V Diagonal Mass', 'viridis', 0, 1),
        ('V_off_diag_mass', 'V Off-Diagonal Mass', 'magma', None, None),
        ('V_diag_ratio', 'V Diagonal Ratio', 'viridis', 0, 1),
    ]
    ...
    fig.suptitle(f'{matrix} | {COMP_LABELS[comparison]} | Alignment Masses (V)', fontsize=14)
    plt.tight_layout()

    out_path = output_dir / f'{comparison}_{matrix}_V_masses_heatmap.png'
...
def plot_diag_mass_all_matrices(
    ...
    """
    Plot V diagonal mass for all 7 matrices in a single figure.
    ...
    for matrix in MATRICES:
        df = load_csv(csv_dir, comparison, matrix)
        sft_data, layers, k_values = pivot_to_heatmap(df, 'sft_V_diag_mass')
        sdft_data, _, _ = pivot_to_heatmap(df, 'sdft_V_diag_mass')
    ...
    fig.suptitle(f'V Diagonal Mass | {COMP_LABELS[comparison]} | Per-Matrix Scale', fontsize=16, y=1.01)
    plt.tight_layout()

    out_path = output_dir / f'{comparison}_all_matrices_V_diag_mass.png'
    plt.savefig(out_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def plot_off_diag_mass_all_matrices(
    ...
    """
    Plot V off-diagonal mass for all 7 matrices in a single figure.
    Each matrix has its own color scale.
    """
    fig, axes = plt.subplots(7, 3, figsize=(16, 28))
    ...
    for matrix in MATRICES:
        df = load_csv(csv_dir, comparison, matrix)
        sft_data, layers, k_values = pivot_to_heatmap(df, 'sft_V_off_diag_mass')
        sdft_data, _, _ = pivot_to_heatmap(df, 'sdft_V_off_diag_mass')
        delta_data = sdft_data - sft_data
    ...
    fig.suptitle(f'V Off-Diagonal Mass | {COMP_LABELS[comparison]} | Per-Matrix Scale', fontsize=16, y=1.01)
    plt.tight_layout()

    out_path = output_dir / f'{comparison}_all_matrices_V_off_diag_mass.png'
    plt.savefig(out_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def plot_matrix_across_comparisons(
    ...
    output_dir: Path,
    matrix: str,
    metric: str = 'V_diag_mass',
):
    """
    Plot a single matrix across all 3 comparisons for V vectors.
...
def main():
    parser = argparse.ArgumentParser(description='Plot alignment mass heatmaps for V vectors')
    parser.add_argument('--csv_dir', type=str,
                        default='eigenspectrum/outputs/alignment_masses/csv',
                        help='Directory containing CSV files')
    parser.add_argument('--output_dir', type=str,
                        default='eigenspectrum/outputs/alignment_masses/heatmaps_V',
    ...
    print(f"Saving V heatmaps to: {output_dir}")

    # 1. Individual matrix heatmaps (all metrics)
    print("\n1. Generating individual matrix heatmaps (all V metrics)...")
    for comparison in COMPARISONS:
        for matrix in MATRICES:
